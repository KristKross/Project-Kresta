...
exports.getInstagramAccountAnalytics = async (req, res) => {
    try {
        ...
        return res.status(400).json({ error: "Missing required parameters: instagramAccountId and accessToken." });
        }

        const response = await axios.get(`https://graph.facebook.com/v22.0/${instagramAccountId}/insights`, {
        params: {
            metric: "accounts_engaged,comments,follows_and_unfollows,likes,reach,replies,shares,total_interactions,views",
        ...
        res.status(500).json({ error: "Failed to retrieve Instagram account analytics." });
    }
};
